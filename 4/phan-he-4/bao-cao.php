<?php
/**
 * Báo cáo tài chính: thu chi theo ngày, top khách hàng, top bồi thường
 * Biểu đồ và so sánh kỳ trước
 */
...
try {
    ...
    $rows_by_day = dbGetAll(
        "SELECT DATE(ngay_gd) as ngay,
         ...
         FROM (
            ...
            FROM PhieuThu WHERE NgayThu BETWEEN ? AND ? AND TrangThai = 'Hoạt động'
            UNION ALL
            SELECT NgayChi as ngay_gd, 'Chi' as loai, SoTien as so_tien
            FROM PhieuChi WHERE NgayChi BETWEEN ? AND ? AND TrangThai = 'Đã chi trả'
         ) combined
         GROUP BY DATE(ngay_gd) ORDER BY ngay ASC",
        [$tu_ngay, $den_ngay, $tu_ngay, $den_ngay]
    );

    // Top 5 khách hàng
    $top_khach_hang = dbGetAll(
        "SELECT k.HoTen, k.SoDienThoai as SDT, SUM(pt.SoTien) as tong_dong
         ...
         GROUP BY k.MaKH ORDER BY tong_dong DESC LIMIT 5",
        [$tu_ngay, $den_ngay]
    );

    // Top 5 bồi thường
    $top_boi_thuong = dbGetAll(
        "SELECT k.HoTen, yc.MoTaSuCo, pc.SoTien, pc.NgayChi
         ...
         JOIN KhachHang k ON h.MaKH = k.MaKH
         WHERE pc.NgayChi BETWEEN ? AND ? ORDER BY pc.SoTien DESC LIMIT 5",
        [$tu_ngay, $den_ngay]
    );

    // So sánh kỳ trước
    $period_start = strtotime($tu_ngay);
    $period_end = strtotime($den_ngay);
    $period_days = max(1, floor(($period_end - $period_start) / 86400) + 1);

    $prev_end = date('Y-m-d', $period_start - 86400);
    $prev_start = date('Y-m-d', $period_start - ($period_days * 86400));

    $thu_prev = (float)dbGetValue(
        "SELECT COALESCE(SUM(SoTien), 0) FROM PhieuThu WHERE NgayThu BETWEEN ? AND ? AND TrangThai = 'Hoạt động'",
        [$prev_start, $prev_end]
    );
    $chi_prev = (float)dbGetValue(
        "SELECT COALESCE(SUM(SoTien), 0) FROM PhieuChi WHERE NgayChi BETWEEN ? AND ? AND TrangThai = 'Đã chi trả'",
        [$prev_start, $prev_end]
    );
    ...
    foreach ($rows_by_day as $r) {
        $labels[] = date('d/m', strtotime($r['ngay']));
        $data_thu[] = (float)$r['thu'];
        $data_chi[] = (float)$r['chi'];
    }

} catch (Exception $e) {
    error_log($e->getMessage());
    die('Đã xảy ra lỗi khi tải báo cáo. Vui lòng thử lại sau.');
}
?>
<!DOCTYPE html>
<html lang="vi">
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/bao-cao.css">
    ...
    <div class="app-wrapper">
        <aside class="sidebar">
            <div class="sidebar-header">
                ...
                <div class="user-info"><?php echo htmlspecialchars($user['name']); ?></div>
            </div>
            <ul class="nav-menu">
        ...
        <main class="main-content">
            ...
            <div class="card-grid" style="margin-bottom: 30px;">
                <div class="stat-card">
                    ...
                    <h3>Tổng Thu</h3>
                    <div class="stat-value"><?php echo formatMoney($tong_thu); ?></div>
                ...
                <div class="stat-card">
                    <div class="stat-icon">💸</div>
                    <h3>Tổng Chi</h3>
                    <div class="stat-value"><?php echo formatMoney($tong_chi); ?></div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value" style="color: <?php echo $loi_nhuan >= 0 ? '#10b981' : '#ef4444'; ?>">
                        <?php echo formatMoney($loi_nhuan); ?>
                    </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value">
                        <?php echo formatMoney($tong_thu / max(1, $period_days)); ?>
                    </div>
                ...
                <div class="card-body">
                    <div class="comparison-box">
                        <div class="stat-card" style="flex: 1;">
                            <h4>Doanh thu</h4>
                            <div class="value"><?php echo formatMoney($tong_thu); ?></div>
                            <div class="<?php echo $tang_giam_thu >= 0 ? 'positive' : 'negative'; ?>">
                                <?php echo $tang_giam_thu >= 0 ? '▲' : '▼'; ?>
                                <?php echo abs(number_format($tang_giam_thu, 1)); ?>%
                            </div>
                        </div>
                        <div class="stat-card" style="flex: 1;">
                            <h4>Chi phí</h4>
                            <div class="value"><?php echo formatMoney($tong_chi); ?></div>
                            <div class="<?php echo $tang_giam_chi >= 0 ? 'negative' : 'positive'; ?>">
                                <?php echo $tang_giam_chi >= 0 ? '▲' : '▼'; ?>
                                <?php echo abs(number_format($tang_giam_chi, 1)); ?>%
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <?php
                                    ...
                                    ?>
                                <tr>
                                    <td><strong><?php echo formatDate($row['ngay']); ?></strong></td>
                                    <td style="color:#27ae60"><strong><?php echo formatMoney($thu); ?></strong></td>
                                    <td style="color:#e74c3c"><strong><?php echo formatMoney($chi); ?></strong></td>
                                    <td style="color:<?php echo $ln >= 0 ? '#3498db' : '#e74c3c'; ?>">
                                        <strong><?php echo formatMoney($ln); ?></strong>
                                    </td>
                                ...
                                <tr style="background:#f8f9fa;font-weight:bold">
                                    <td>TỔNG CỘNG</td>
                                    <td style="color:#27ae60"><?php echo formatMoney($tong_thu_ct); ?></td>
                                    <td style="color:#e74c3c"><?php echo formatMoney($tong_chi_ct); ?></td>
                                    <td style="color:#3498db"><?php echo formatMoney($tong_thu_ct - $tong_chi_ct); ?>
                                    </td>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="color:#27ae60">
                                        <strong><?php echo formatMoney($r['tong_dong']); ?></strong>
                                    </td>
                                </tr>
                                <?php endforeach; else: ?>
                                <tr>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="color:#e74c3c"><strong><?php echo formatMoney($r['SoTien']); ?></strong>
                                    </td>
                                    <td><?php echo formatDate($r['NgayChi']); ?></td>
                                </tr>
                                <?php endforeach; else: ?>
                                <tr>
