...
use UngDung\KhoDuLieu\PhieuThuKho;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'create') {
        $duLieu = [
            ...
            'ngay_thu' => $_POST['ngay_gd'] ?? date('Y-m-d'),
            'ghi_chu' => trim((string) ($_POST['ghi_chu'] ?? '')),
        ];
        $ketQua = $phieuThuKho->tao($duLieu, $nguoiDung ?? []);
        $message = $ketQua['thong_diep'];
        $message_type = $ketQua['ok'] ? 'success' : 'error';
        if ($ketQua['ok']) {
            $nhatKy->ghi('phieuthu', (string) ($ketQua['ma_pt'] ?? ''), 'CREATE', null, $duLieu, $nguoiDung['ma_nv'] ?? null);
        }
    } elseif ($action === 'edit') {
        $duLieu = [
            'ma_pt' => $_POST['ma_gd'] ?? '',
        ...
        $ketQua = $phieuThuKho->capNhat($duLieu);
        $message = $ketQua['thong_diep'];
        $message_type = $ketQua['ok'] ? 'success' : 'error';
        if ($ketQua['ok']) {
            $nhatKy->ghi('phieuthu', (string) $duLieu['ma_pt'], 'UPDATE', null, $duLieu, $nguoiDung['ma_nv'] ?? null);
        }
    } elseif ($action === 'delete') {
        $maPt = $_POST['ma_gd'] ?? '';
        $ketQua = $phieuThuKho->xoa($maPt);
        $message = $ketQua['thong_diep'];
        $message_type = $ketQua['ok'] ? 'success' : 'error';
        if ($ketQua['ok']) {
            $nhatKy->ghi('phieuthu', (string) $maPt, 'DELETE', null, null, $nguoiDung['ma_nv'] ?? null);
        }
    }
}
...
$list_phieuthu = $phieuThuKho->danhSach($boLoc);
$tong_thu = $phieuThuKho->tongThu($boLoc);
$list_hopdong = $phieuThuKho->danhSachHopDong();
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <select name="ma_hd" id="ma_hd" required onchange="autoFillAmount(this)">
                                        ...
                                        <?php foreach ($list_hopdong as $hd): ?>
                                        <option value="<?php echo htmlspecialchars($hd['MaHD']); ?>"
                                            data-phi="<?php echo htmlspecialchars($hd['PhiBaoHiem']); ?>">
                                            HD-<?php echo str_pad($hd['MaHD'], 3, '0', STR_PAD_LEFT); ?> |
                                            <?php echo htmlspecialchars($hd['HoTen']); ?> |
                                            <?php echo htmlspecialchars($hd['BienSo']); ?> |
                                            Phí: <?php echo DinhDang::tien((float) $hd['PhiBaoHiem']); ?>
                                        </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
            ...
            <div id="tab-list" class="tab-content">
                ...
                <div class="stat-summary">
                    <div class="stat-item">
                        ...
                        <div class="value"><?php echo count($list_phieuthu); ?></div>
                    </div>
                    <div class="stat-item">
                            ...
                            <table>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td style="color: #10b981;"><strong><?php echo DinhDang::tien((float) $pt['SoTien']); ?></strong>
                                        </td>
                                        <td><?php echo DinhDang::ngay($pt['NgayThu']); ?></td>
                                        <td><span class="badge badge-success">Hoạt động</span></td>
                                        <td>
                                            <div class="action-buttons">
                                                <button onclick='openEditModal(<?php echo json_encode($pt); ?>)'
                                                    class="btn btn-sm btn-warning">✏️ Sửa</button>
                                                <button
                                                    onclick="confirmDelete('<?php echo $pt['MaPT']; ?>', 'PT-<?php echo str_pad($pt['MaPT'], 4, '0', STR_PAD_LEFT); ?>')"
